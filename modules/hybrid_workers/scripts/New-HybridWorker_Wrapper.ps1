...
if($env -eq "nonprod"){$subID = "NONProd-SubID"}
#Prod-CorporateServices-VDI 
if($env -eq "prod"){$subID = "PROD-SubID"}

Connect-AzAccount
Set-AzContext -Subscription $subID | Out-null
#Connect-AzureAD 

$ResourceGroupName = (Get-AzResourceGroup -Name "rg-$($env)-mgmt").ResourceGroupName
$AAName = (Get-AzAutomationAccount -ResourceGroupName $ResourceGroupName -Name "aa-$($env)-001").AutomationAccountName
...
$WorkspaceName = (Get-AzOperationalInsightsWorkspace -ResourceGroupName $ResourceGroupName -Name "la-$($env)-automation").Name
